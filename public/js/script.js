// Map categories to SVG icons used in the step 5 summary screen
const CAT_ICON = { hardware: '#i-cpu', software: '#i-apps', network: '#i-wifi', account: '#i-key' };
let cur = 1;

/* ── 1. Step Control Navigation ── */
function setStep(n) {
  // Hide current panel, show the next one
  document.getElementById('panel' + cur).classList.remove('active');
  document.getElementById('panel' + n).classList.add('active');
  
  // Synchronize step tracking wizard bubbles at the top
  for (let i = 1; i <= 5; i++) {
...
/* ── 2. Validation Checks per Panel Step ── */
function goStep(from) {
  ...
  if (!ok) {
    document.getElementById('err-msg').textContent = 'Please fill in all required fields before continuing.';
...
/* ── 3. Live Cascading Location Ajax Data Fetches (FIXED ROUTES) ── */
...
function onCampusChange() {
  ...
  // Wipe dropdowns cleaner downstream
  ...
  // FIXED: No leading slashes to prevent dynamic route drops
...
/* ── 4. Review Panel Compilation Node Mapping ── */
...
/* ── 5. Form Exit & Submission Actions ── */
function trySubmit() {
  ...
  // Submit the real HTML form data fields to Laravel backend controller
  form.submit();
}

function handleExit() {
  if (confirm("Are you sure you want to exit? Your current ticket draft will be lost.")) {
    // Redirects customer safely back to their main ticket list overview dashboard
